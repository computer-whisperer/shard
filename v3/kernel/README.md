...
| `record.shard` | `RecDef RxRes` | **phase 3, slice 3.9 (2026-09-17):** records as loader-level sugar (`v3/LANGUAGE.md` §4, §13 item 42): `expand_records` turns each `(record …)` into its positional type, `FIELD_of` and `with_FIELD`, and rewrites `make` and `with` in every form, before any form is read; the bootstrap's `load.rs` `expand_records` is the twin, parity the tie |
