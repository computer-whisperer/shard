...
| `level.shard` | `Level` | `normalize`, `is_equivalent`, `is_geq`, `is_not_zero`, `normalizes_to_zero`; `check_level` (declared parameters only); `LMVar` (slice 3.15) is the elaborator's universe metavariable — an atom to every rule here, as level.cpp's, and never accepted by K |
| `expr.shard` | `BinderInfo Literal D Expr Memo St` | the typing rules per constructor (`infer_*`), `whnf_core`/`whnf`, definitional equality (`is_def_eq_core` in its exact order), literal rules, projection rules; the representation (node data, identity, the state); `ingest_expr` — a raw term's node data is a claim, rebuilt at K's raw entry `check` (add.shard; §3.5, GPT-6 R43, 2026-09-12); `MVar` (slice 3.15) is the elaborator's metavariable with `has_mvar` in the node data (levels included) — `infer` refuses the node (`metavariable`), the raw entry the declaration (`mvar_in_type`/`mvar_in_value`; hostile 17a–d) |
...
| `record.shard` | `RecDef RxRes` | **phase 3, slice 3.9 (2026-09-17):** records as loader-level sugar (`v3/LANGUAGE.md` §4, §13 item 42): `expand_records` turns each `(record …)` into its positional type and an accessor `NAME.FIELD` and updater `NAME.with_FIELD` per field, and rewrites `make` and `with` in every form, before any form is read; the bootstrap's `load.rs` `expand_records` is the twin, parity the tie (`dump.shard` prints a head by its declared spelling, `dm_head`, so two records' `x` stay distinct) |
...
| `scope.shard` | `Open Vis Scope Cx Defd Resolved` | **slice 3.15 (2026-09-18), out of reader.shard:** names, the loader's scope and resolution (§3.1: the module's own declarations, the opened prefixes in order, the bare name; two hits = ambiguous; `Init.NAME` the explicit citation; in an E-type position a type over its same-named constructor); `Cx` = scope, environment, universe parameters, the names the form declares with their types (`Defd`, citable before admission), the bound names with their fvars, the projection aliases with their types, the E-type flag; `type_head`, `def_height`; **slice 3.16:** `resolve_ctor` — a constructor by the type a position expects (the name under the inductive's own prefix first, then the scope's candidates): every pattern head, and a term's ambiguous head |
| `kw.shard` | `W WOut` | **slice 3.15, out of erase.shard:** a walk over K's terms from outside the seal — the environment, a local context and the checker state with K's procedures through the view (`w_whnf`, `w_infer`, `w_is_def_eq`, `w_is_prop`, `w_local`/`w_local_bi`/`w_local_val`, `w_pis`/`w_lams` = K's `mk_binding`, `w_subst`); every translator and the elaborator build over it; **slice 3.16 (rule 9):** `w_infer_r`/`w_whnf_r`/`w_is_def_eq_r` keep K's outcome (`WVal`, or `WFail` with K's `Failure` — a refusal's reason or a spent resource's site; `failure_spent`), the Option forms below them for the erasure and the translators; `w_admit` checks a generated declaration into the walk's environment (K's `check` seeded above the walk's node ids, the walk's raised above the new environment's); `reason_of` moved here from the loader |
| `unify.shard` | `MDecl MCtx URes` | **slice 3.15 (law §6, §5.1):** metavariables and first-order unification — a metavariable's declaration (name, type, telescope, kind), the assignments, `inst_mvars`/`inst_level` (instantiate), `first_mvar`/`first_lmvar` (the unsolved), `unify` (structural descent assigning after an occurs check and the scope check, the assignment typed — `infer_light` on an open value —, levels structurally with `is_equivalent` closed, K's `is_def_eq` on a closed mismatch, one retry after K's whnf; transactional), `mc_drop_fvars` at a binder close; **slice 3.16 (rule 9):** `URes` = `UYes`, `UNo` (never inequality), `UStuck i` (blocked on an unassigned metavariable applied to what is no pattern), `UExhausted site`; an assignment typed or tentative (`mc_tentative`, `tentative_bad`), a closed value K refuses not installed unless it cites a constant still being declared (`cites_undeclared`); `unify_depth` for the budget test |
| `elab.shard` | `El Gen EU ERes EArg Slot TlRes BRes FinRes PtRes` | **slice 3.15 (§5.1 Stage 1):** the L elaborator — bidirectional over K's locals; a constant's telescope into slots (`tele`: a metavariable per implicit binder, a placeholder per written argument), `elab_args` (the expected type propagated first, the arguments at their binders, numerals last, the placeholders unified), `check_expected` (unify, else the one coercion `Nat → Int`, else `type_mismatch`), universe inference (a level metavariable per parameter not written; level 0 by rule in E-type positions), `@NAME`, the operator spellings by the first operand's type (`elab_op`, `op_identity`), `if` as `ite` with the decision of the proposition's head (`decision_of`) or `cond`, `exists`, binders/arrows/lets closed by K's `mk_binding` with the Pi's sort computed here (a form may cite the constant it declares), `el_finish` (the closed term; an unsolved metavariable named: `unsolved_implicit`, `instance_needed`, `unsolved_universe`); **slice 3.16 landing 1:** `elab_match` and `el_pat` (a `match` as a generated matcher applied to the constant motive, the scrutinee and an alternative per row; the matcher checked into the walk's environment and kept in `Gen` for the reader — `el_aux`), `expected_ctor` (rule 5), a body's untyped `let`, `el_unify_r`/`eu_refusal` (`unify_stuck`, `unify_exhausted`), `el_finish` refusing a bad tentative assignment; **landing 2:** `decide` and `= true` in `check_expected`, `if` on a `Bool` as `ite (c = true)` and on a two-constructor inductive as its `match` (`two_ctors`), the operator's type past a numeral operand, `elab_list`, implicit insertion for a bound head |
| `matcher.shard` | `MtPat MtRow MtRes MtGen MtInd MtPt MtAlts MtDesc` | **slice 3.16 landing 1 (§8.4 rule 1; GPT-6 R64):** matchers in Lean's shape — `mt_generate` (motive, scrutinees and alternatives as K locals; `mt_compile`: the `casesOn` tree by first-match column splitting, a fall-through alternative applied at every leaf it reaches, a variable bound before a later split refined by it), and the description read back: `mt_describe` (the rows off a matcher's type) and `mt_is_matcher` (the definition regenerated from them and compared with the admitted value — a name decides nothing) |
...
| `erasure.shard` | `Tr TrRes Field EKind ErEnt ErRes PdErase Either5r` | **slice 3.16 landing 3 (split out of realize.shard):** the E projection — the walk's state `Tr` and K's procedures over it (`tr_infer`, `tr_whnf`, `tr_local`, `tr_lams` …; a definition's value and descent obligations ride it); `rz_open_ctor`; the erasure `erase` (§7.1, §7.5): constructors and calls at their runtime arguments, `casesOn`/a recursor without hypotheses as a match, a two-constructor case analysis ignoring its fields as the `if` it was written as (`if_branches`), `ite`/`dite`, a matcher application as the match of the rows read back (`erase_matcher`), `decide` as the explicit conversion, the realization registry (`realization_of`: `Int`'s and `Nat`'s integer entries); `erase_predef` — a pre-definition's program, the self-local and a callee-local a call, the recursion clause erased (`predef_rec`). Untrusted; a stated bring-up trust dependency (R58) |
| `define.shard` | `DfRes TrAux PjKind PjCx` | **phase 3, slices 3.13–3.14; rebuilt at slice 3.16 landing 3 (2026-09-19):** Stage 1's `fn` = `def` + `realize` — the **K projection of a pre-definition** (`v3/LANGUAGE.md` §8.4's landing-3 as-built; §13 items 43–45, 47, 49). `define_pd`: a body that never cites the self-local is the value under its binders; structural recursion is `T.brecOn` with each matcher kept as the constant K checked and its motive carrying the course-of-values tables its scrutinee refines (`pj_match`, Lean's addArg), a self-call the table's entry found by type over an abstract motive (`pj_find`, `pj_search`); a measure is `WellFounded.fix` over the data binders' tuple, a self-call `rec (tuple args) (f.dec_N locals…)` over every local in scope, `ite` as `Decidable.rec` where a self-call sits under it (`pj_ite`); a callee-local in the body is the obstacle `callee_runnable`. The definition is checked into the walk's environment and the equations stated where K decides them (realize.shard's `pj_equations`). `DfSkip` obstacles: `below_unreached`, `wf_unreached`, `measure_type`, `callee_runnable`. Also here, unchanged: a native inductive's `below`/`brecOn` (slice 3.14) and its constructor-structure bundle (slice 3.15). Deleted at landing 3: the E-term matrix and table bookkeeping (`df_*`, `Known`, `DfSig`, `DqRes`) |
| `realize.shard` | `RzRes RtRes PjLeaf PjFo Either3 Either4` | **slice 5b (§7.5); rebuilt at slice 3.16 landing 3 (rule 7):** `realize` — the supplied form: the E signature against the L type's erasure (`signature_matches`), the signature's names bound to the constant's own telescope (`bind_tele`), the body elaborated once at the result's L type, its erasure the program, descent and order checked on it (`check_calls`), the matchers it generated returned for admission; the derived view (`realize_view`). **The equations** (`pj_equations`, shared with define.shard): the leading matchers' rows against the scrutinee term — bind, clash, stuck-and-split (`pj_fo`, `pj_split`) — one theorem per leaf; a `fn`'s stated where K decides it (`pj_decided`), a realization's always, by `Eq.refl` or the `equations` clause. Deleted at landing 3: `tr` (the upward E-to-L translation) with the typed application through `c_type`, the E-term case tree, the old equations, `DefRec`/`BelowInfo`/`WfInfo` |
...
18. **Metavariables and free variables** in inputs are refused (`check_no_metavar_no_fvar`; the `MVar`/`LMVar` nodes exist since slice 3.15 for the elaborator above K, LANGUAGE.md §8.4, and K refuses every term carrying one); `mdata` is transparent to every rule and is stripped at import (the exporter drops it by default).
