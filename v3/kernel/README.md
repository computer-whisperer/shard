...
| `loader.shard` | `Mod InitSt Rec Load Fx Form` | **phase 2, slice 3 (2026-09-12):** the loader (`v3/LANGUAGE.md` §3, §3.1–3.3, §9) — a package root and its files as modules; `import` once, cycles refused, the imported closure visible and nothing opened; `(import Init NAME)` streaming the pinned export through NAME against the pin's meta line, nested prefixes loaded once; `use` and selective `use`; `trusts` as the file's policy over each admitted declaration's axiom closure; `sorry` pending; the acceptance records (`Rec`) |
| `load.shard` | — | the loader's driver: `eval direct v3/kernel/load.shard --root DIR [--init CHUNK]… [--route N] [--engine S] FILE…`; prints the records and the counts; exit = refused + errors |
