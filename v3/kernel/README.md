...
| `define.shard` | `DfRes DqRes DfSig Known TrAux` | **phase 3, slices 3.13–3.14 (2026-09-17):** Stage 1's `fn` = `def` + `realize` (`v3/LANGUAGE.md` §8.4, §13 items 43–45, 47): a classified fn whose signature's types and body's heads have L identities gets a `DefnDecl` — the leading case tree as nested recursors with a constant motive (`df_matrix`: realize.shard's rows and columns, a known constructor split statically); a structural recursion is `T.brecOn` of a functional whose table every split of a table-owning variable generalizes (`df_brec`, `df_split_tab`: the entries read positionally in Lean's shape), a native inductive's `below`/`brecOn` generated on first need (`gen_below`, admitted first through `DfAux`); a measure is `WellFounded.fix` over the parameter tuple under `InvImage Nat.lt_wfRel.rel m` (`df_wf`), each self-call's decreasing fact an obligation (realize.shard's `tr_self_wf`) — then its equations `NAME.eq_N` through `equations` (none for a measure); the type parameters implicit binders (slice 3.15), so a theorem cites `(len xs)`; **slice 3.15:** `confusion_bundle` — after a native inductive's admission, `T.casesOn` (`gen_cases_on`: the recursor with the hypotheses unused, universe polymorphic) and, for a type without parameters, `T.noConfusionType`, `T.noConfusion` (`gen_nct`, `gen_nc`: Lean 4.33's shapes off the export) and `T.c.inj` (`gen_injs`: a term proof through `noConfusion`, the fields implicit); an obstacle (`df_obstacle`) is `DfSkip` (RUNNABLE, the reason named), any other failure `DfErr` (refused) |
| `realize.shard` | `Tr DefRec BelowInfo WfInfo EV NPat Col Row Leaf Field RzRes …` | **slice 5b (§7.5):** `realize` — the supplied form: the E signature against the L type's erasure, the body as a `fn`'s, descent (a self-call passes a variable bound under the structural parameter) and order (a callee realized earlier, or a sig fn), the leaves of the body's leading matches compiled column by column (a column's variable replaced by the constructor term everywhere, a variable row bound to it), each leaf an equation `NAME.realize_N` — the body translated into L with every constant's telescope filled by first-order matching, a `let` by inference, a non-variable `match` and an `if` as the recursor with a constant motive — proven by `Eq.refl` or the `equations` clause; the derived view: the value's lambdas the binders, `casesOn`/`rec` (no hypothesis used) a `match`, `ite`/`dite` an `if`, a projection a `match`, constructors and realized constants by their roles, `brecOn` and choice refused by name; **slice 3.16 landing 2:** the erasure's new rules — a matcher application to the `EMatch` of the rows read back (`erase_matcher`, `mt_epat`), the self-local to an `ECall` (`EKCall`), `decide` to `(if ⟦inst⟧ true false)`, `instDecidableEqBool c true` to `⟦c⟧`, `Int.ofNat`/`Int.negSucc`, the `Int` rows of the realization registry (`realization_of`); `erase_predef`; `tie_check`/`tie_term`, landing 2's alarm (the erasure against the classifier's program), gone with `tr` at landing 3 |
