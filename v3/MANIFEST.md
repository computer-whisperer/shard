...
| `examples/calc` | — | 3,130 | PORT — the program half (lexer, parser, evaluator, show, the app's step; 51 fns, 9 types, ~1,000 lines) **PORTED 2026-09-13 (slice 6) to `v3/examples/calc/`, one S file per old file, byte-identical to the old tree over the differential's input set**; the 100 claims (~2,100 lines) at phase 3 with tactics (ruled 2026-09-07) | `examples/calc/calc_differential.shard` is the old side's pure driver module |
