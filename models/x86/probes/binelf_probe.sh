#!/usr/bin/env bash
# models/x86/probes/binelf_probe.sh — the hand-driven BIN-rung de-risk probe (docs/X86.md
# §21). Emits the two-segment ELF from models/x86/probes/binelf_probe.shard, then on real
# Linux:
...
ARG=hello  # must match tstr in models/x86/probes/binelf_probe.shard
...
"$EVAL" run models/x86/probes/binelf_probe.shard > "$TMP/probe.txt" || {
