...
> **STATUS (reset 2026-08-22): LAW.** the measure-descent admissibility system, built and corpus-pinned. The backlog is the GitHub issue tracker (labels `arc:coverage` / `parked` / `debt`; the goal = #23, the prune arc = #24) — any "next arc/rung" pointer below is history unless it names an issue.
...
**Where the certs live — the two-home rule (friction item 5, 2026-07-10).**
An Int measure's obligations (nonneg + decrease per site) ALWAYS require
certs; check time never searches. The certs live in exactly one of two homes:

- **inline** in the clause — `(measure (- n i) (by arith …) (by arith …))` —
  self-contained, hand-authored; the style of proof-dense model files
  (bytetie, lowbuild, x86gen).
- **the machine-owned sidecar** — a bare `(measure (- n i))` clause pulls each
  site's proof from `<file>.auto.shard` under the key
  `<fn>#<site>#decrease|nonneg`, generated and regenerated by
  `tools/prove FILE` (wasmgen's `xsym_idx` is the standing example; prove
  re-derives its entries byte-identically). A missing entry is a hard
  failure, never a silent pass.

Both styles are canonical; the sidecar IS the auto-derive path — no kernel
involvement, verify-don't-search intact. Prefer it for mechanical Int
measures; keep inline where clause-local readability matters.
