...
**C2b-5 — DESIGN RECORD (2026-09-05; written before the first edit):
rt_dec's engine leg — the release theorem assembled.** WHAT THE LAW
SAYS. Calling rt_dec (v) on a heap whose bytes mean ghost g
(heap_rep), well-formed at roots r (hinv), with the raw cell CLOSED
(`graw_of g = None` — hinv_dec's premise; the fill window never
releases), where v is an OWNED reference or an immediate (`memb r v`
for the reference legs — under roots_ok that makes v a live address),
always RETURNS 0 and leaves memory `m_dec m g v` whose bytes mean
`gh_dec g v`, and the program owns one reference fewer: hinv holds at
roots `irem r v` (hinv_dec, C2b-2). Legs in the code's case order:
IMMEDIATE (v odd): identity. SATURATED (count ≥ 2^31): identity, yet
the roots still lose v (the immortal exemption, C2b-4's mirror).
SHARED (1 < count < 2^31): the one header store `[v] := hword − 1`
(ghost `hdecc`) — the exact mirror of rt_inc's reference leg. LAST
(count = 1): THE CASCADE — `[v] := hword − 1` (the count band becomes
the worklist LINK, 0 = end), then the outer loop pops the head, reads
its arity from the header's high band and its link from the low band,
walks its slots (the inner loop): an odd slot is skipped, an even slot
w loads `[w]`: immortal → skip; shared → `[w] := header − 1`; dying
(count 1) → `[w] := header − 1 + link` and w becomes the head (LIFO,
pinned by rth_run 2026-08-23); after the slots, a cell of arity < 16
goes to its free class (`[a] := class head; [free_at rb arity] := a`,
= fls_push), an arity ≥ 16 cell is abandoned (the ghost drops it from
cells and pushes nothing — the same leak shape the ghost already has).

**THE MEMORY MIRROR (new vocabulary, rth_kit).** `m_slots m ws a i
cells wl` / `m_wl k m cells fls wl` / `m_dec m g v` — the memory
effect written in gh_slots / gh_wl / gh_dec's OWN recursion (the same
budget k, the same case ladder, the store where the ghost mutates),
so each engine lemma's rhs is one ghost step's memory and the
assembly is `heap_rep (m_dec m g v) rb (gh_dec g v)` (hr_dec).
`hwl_rep m wl` — the worklist's representation: for each dying cell d
in wl, `[haddr d] = hword d − 1 + (haddr of the next, 0 at the end)`
and d's slots intact at haddr d + 8 (dying cells are OUT of cells and
so out of hcells_rep; this predicate carries them). Evidence before
theorems (C2b-2's rule): rth_run gains rows comparing rt_dec's actual
bytes against `m_dec`'s on scenarios 1–3 (chain cascade, shared
parent, shared child) BEFORE any lemma is stated.

**THE INVARIANT (the outer loop, at ghost budget k, ghost state
(cells, fls, wl), locals lc with lc[3] = haddr of wl's head or 0):**
`heap_rep m rb (g ⟨cells, fls⟩)` ∧ `hwl_rep m wl` ∧ `winv`/`wexact`
(C2b-2's pure invariant — gh_wl_inv / winv_pop / winv_push /
gh_slots_inv are the ghost half, already proven) ∧ the address facts
(every wl address and every cells address is aligned, in
[glo, top), pairwise extent-disjoint: winv's e_disj over cells ∪ wl
extents). The INNER loop's invariant at slot index i over cur's slot
list: the outer invariant with (cells, wl) = `gh_slots (first i
slots)` and lc[6] = i, lc[4] = haddr cur, lc[3] = head. Both loop
lemmas are wf-inductions: the inner on the remaining slot list (the
ghost's `gh_slots` recursion), the outer on the ghost budget k (the
ghost's `gh_wl` recursion). The IH is cited after ONE iteration's
straight-line stepping (rt_inc's recipe per statement: compute to the
stuck guard, the band/alignment facts from `live_ok`'s peels, the
frame toolkit for every store, hcells_rep_decc — hcells_rep_incc's
mirror — for a shared child's header store, a NEW `hwl_rep_push` for
a dying child's header store, `hwl_rep_fr` framing for the stores at
other addresses). FUEL: ipwhile burns one level per ITERATION
(`ipwhile f2` recurses at the decremented fuel) and the body runs at
that level; the budget is `k_outer + 16 + c` (c = the entry's nesting,
found empirically as C2b-3/4 did), linear in the ghost budget —
stated as a `ntl`-tower premise like rt_inc's `nS 12 (ntl 12 fuel)`
with the tower height a function of k.

**THE PARTS.** (i) vocabulary + differential: m_slots/m_wl/m_dec,
hwl_rep, the rth_run rows; then the THREE SIMPLE LEGS generated by a
`dec` block in gen_rth.py (imm / sat / shared — rt_inc's three legs
with the count test flipped; hcells_rep_decc + counts_ok's dec
bookkeeping from C2b-2's hdecc laws). (ii) the inner loop:
`rth_slots_run` (one full slot walk = gh_slots + m_slots) with its
per-child lemmas. (iii) the outer loop `rth_wl_run`, the LAST leg's
entry, and the assembly `rth_dec_run` + `hr_dec` + the corollary at
`irem r v`. Each part: rth_kit 0 failed locally, rth_run's rows green,
CI FAIL set == baseline, a LANDS paragraph. RISKS NAMED: the worklist
link lives in the count band, so `hwl_rep`'s header equation mixes
hword's bands with an address — `hword_count`/`hword_hi` and the
2^32 address bound (hinv's `gend ≤ 2^32`) keep it a plain sum; the
inner loop's IH needs the outer state's e_disj to frame each child
store away from cur's own slots (a child is never cur: cur is out of
