...
| R45 runnable Stage-0 programs distinct from admitted definitions and checked realizations | accept — **status kind and two of the three tests landed 2026-09-12 (slice 5)**: `RUNNABLE fn NAME` per declaration, distinct from `ACCEPT`; `ev_test` (the self-recursive candidate exhausts at any fuel, never a value) and `v3/pins/loader/ev_no_l_meaning` (a theorem citing a `fn` finds no constant); the third test waits for `realize` (slice 5b) | `LANGUAGE.md` §0, §6.2, §6.7, §7; a status kind in the driver's per-declaration output; the three named tests (a self-recursive candidate exhausts and gains no equations; a value discharges no proof requirement; a checked realization distinguishable from a linked function) | ruling A restated as API discipline; nothing new to decide |
