...
| R47 prefix imports and compatibility omissions as bounded bring-up mechanisms | accept — wording | `LANGUAGE.md` §3: an imported declaration's identity is the pin plus its name plus its content; the prefix is only the scope the load recorded, so enlarging it changes no earlier declaration; §12's AT RISK rows gain owner, consumer and regression | the draft had conflated the load with the declaration |
| R48 ratified rules vs demonstrated coverage vs open obligations | accept — status text | FOUNDATION's banner → a pointer to `v3/README.md`; `v3/README.md` lists the open obligations beside the phase-1 result (R42 until green on CI, R43 until enforced, the Stage-0 `fn` gap) | the banner still said phase 0 open and nothing implemented |
