...
5. **The fixed-identity primitives** (`v3/kernel/accel_pins.shard`,
   GENERATED by `gen_pins.sh` from the pinned export, never hand-edited):
   K enables an accelerator, a literal typing rule or a literal
   expansion only for a declaration whose admitted identity (the
   structural hash of its type and value) matches its row; a
   same-spelled non-core `Nat.add` never reaches a shortcut.
