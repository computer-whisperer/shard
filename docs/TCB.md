...
> **STATUS (reset 2026-08-22): LAW.** the trust story — exactly what a verdict rests on. The backlog is the GitHub issue tracker (labels `arc:coverage` / `parked` / `debt`; the goal = #23, the prune arc = #24) — any "next arc/rung" pointer below is history unless it names an issue.
...
> **FOUNDATION (2026-09-06): AMENDED — this roster governs the OLD tree; `docs/FOUNDATION.md` §9 governs `v3/`** (the V3 roster — K, the Rust host, the oracle evidence, retained P, the fixed-identity primitives — is written below (phase 1, 2026-09-07) and replaces this file at the flip; §10.5). **The translations trusted during V3 bring-up** (FOUNDATION §12.4 phase 0), each by review until the named gate retires it: (1) the Rust bootstrap as E's executor, as today (retired by route 1, §9.1, never by fiat); (2) the Rust narrow loader for the V3 toolchain's own sources, in exactly today's parsing profile (§9.2) — **RETIRED 2026-09-13 (phase 2, slice 6): `v3/kernel/test/parity_test.sh` has the V3 reader re-parse every toolchain closure byte-identically on every run (18 closures, 61,080 declaration lines), so the Rust loader's parse is cross-checked, never trusted alone;** since phase 3's slice 3.2 (2026-09-14) the loader reads the one E of `v3/LANGUAGE.md` §8 — explicit type binders, sequential `let`, directory imports, the L forms skipped, dotted citations — cross-checked the same way over 21 closures, one with a directory module; (3) the `lean4export` reader and the import identity mapping into `Env` (retired by T0: declaration-for-declaration agreement with the export and the independent checkers); (4) the initial S→L elaborator for the toolchain's own inline proofs, which are pending claims under the reviewed host until K checks them (§9.2); (5) the old tree's checker as the differential **oracle** for ported modules — evidence, never authority; (6) the temporary native chain for speed, never authority, exactly as this file already says; (7) **dated 2026-09-14 (phase 2, slice 10; GPT-6 R58):** the erasure of a derived `realize` view and — since slice 3.16 (2026-09-19), when the E→L translation of a supplied realization's equations was deleted — of every `fn`'s and supplied realization's pre-definition, which **is** its program (`v3/kernel/erase.shard`, `v3/kernel/erasure.shard`; the equations are now read off the elaborated L body in `v3/kernel/realize.shard` and checked by K), reviewed — a `REALIZE NAME view` record says which rule the body follows, not that the translation was verified; retired when a checked translation or route 1's proof covers them (phase 3 or later). Nothing else is trusted during bring-up; an eighth item is a dated decision.
...
## The V3 roster (FOUNDATION §9, phase 1 — written 2026-09-07)

A V3 verdict (`v3/`, the sibling tree; FOUNDATION §12.1) rests on, in
order — this roster replaces the one above at the flip (§10.5) and
governs `v3/` now:

1. **K** (`v3/kernel/*.shard`): the rule inventory as declarations in
   first-order shard, Lean-rule-exact against the pinned release
...
2. **The Rust host** (`rust_bootstrap`, `eval direct`): route 3 of §9.1
   ...
   AUTHORITY for V3 verdicts. Its parse of the toolchain's own sources
   is cross-checked by the V3 reader on every test run (frontend
   parity, `v3/kernel/test/parity_test.sh`, 2026-09-13). The compiled K (`v3/bin/t0`, built by the
   temporary chain — route 1 without its proof, ruled 2026-09-07) is a
   differential artifact: it runs T0's full replay only after the
   interpreter byte-ties its verbose output on a prefix, and its word
   is never authority.
3. **The oracle evidence** — T0 (§3.6): the pinned `lean4export` of
   Lean's `Init` (v3/README.md pin; `v3/export.sh`) replayed
   ...
   and the toolchain's tests (`v3/test.sh`) — since phase 2
...
4. **Retained P** (§7.5): none yet — the first release bundle with
   retained proof evidence is phase 3.
5. **The fixed-identity primitives** (`v3/kernel/k/accel_pins.shard`,
   GENERATED by `gen_pins.sh` from the pinned export through
   `refgen.shard`, never hand-edited): K enables an accelerator, a
   literal typing rule or a literal expansion only for a declaration
   whose admitted **identity closure** — itself and, transitively, every
   definition, opaque, axiom and inductive type its type and value cite
   — matches the reference declarations exactly, by structural equality
   (kind, level parameters, type, value; an inductive's parameter count,
   block and constructors in order; theorems excluded by proof
   irrelevance, constructors and recursors through their inductive).
   Never a hash and never a name: the 61-bit structural hash that
   authorized pins until 2026-09-12 was affine in a trailing literal
   (GPT-6 R42, `records/FOUNDATION.md` §4.7); a same-spelled non-core
   `Nat.add`, the one-subtraction collision, and the genuine body over a
   re-spelled dependency all reach no shortcut (hostile battery 7,
   7b–7d).

Not on the V3 roster: the C compiler (cc), the temporary chain
(tools/lower, tools/codegen), the test scripts, GitLab — as above, they
orchestrate or accelerate; the verdict is the interpreter's on K.
...
**V3 (phase 1, 2026-09-07):** for `v3/` the temporary-chain lifetime
story above is replaced by FOUNDATION §9.1's four routes — (1) K
compiled by shard's own lowering, a proven artifact; (2) K under
compiled `ev`; (3) the Rust bootstrap directly; (4) the full tower as
the differential, never mandatory — with the route recorded per
acceptance (§8.1). Until route 1 carries its proof, the chain's compiled
K is confirmed by byte-tie against route 3, exactly the discipline this
section describes for the old tree.
