#!/usr/bin/env bash
# lowbuild_x86call.sh — the x86_64 CALL-fragment artifact build (docs/X86.md
# §16): the mod.build convention end to end for examples/x86call_src.shard,
...
set -euo pipefail
SRC=examples/x86call_src.shard
OUT=examples/x86call_out.shard
BUILD=examples/x86call_src.build.shard
EVAL=bin/shard_eval
...
echo "ARTIFACT OK: call-fragment binaries + manifest + certs, all five gates green"
