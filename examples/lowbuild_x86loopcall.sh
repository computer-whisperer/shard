#!/usr/bin/env bash
# lowbuild_x86loopcall.sh — the x86_64 CALLS-IN-LOOPS artifact build (docs/X86.md
# §18): the mod.build convention end to end for examples/x86loopcall_src.shard,
...
set -euo pipefail
SRC=examples/x86loopcall_src.shard
OUT=examples/x86loopcall_out.shard
BUILD=examples/x86loopcall_src.build.shard
EVAL=bin/shard_eval
