#!/usr/bin/env bash
# lowbuild_x86intloop.sh — the x86_64 INT-RETURN-loop-fragment artifact build (docs/X86.md
# §14 growth): the mod.build convention end to end for examples/x86intloop_src.shard,
...
#                  (examples/x86_diff.c: mmap + SysV call; XMEMCASE
#                  readback windows against the zeroed data page);
#                  ARTIFACT lines are filtered out of the replayer's copy
# Exit 0 = a fully gated artifact set. Run from the repo root.
set -euo pipefail
SRC=examples/x86intloop_src.shard
OUT=examples/x86intloop_out.shard
BUILD=examples/x86intloop_src.build.shard
EVAL=bin/shard_eval
...
echo "ARTIFACT OK: Int-return-loop binaries + manifest + certs, all five gates green"
