#!/usr/bin/env bash
# lowbuild_x86intloop.sh — the x86_64 DIV-fragment artifact build (docs/X86.md
# §41, the W4 emitter slice): the mod.build convention end to end for examples/x86itoa_src.shard,
...
set -euo pipefail
SRC=examples/x86itoa_src.shard
OUT=examples/x86itoa_out.shard
BUILD=examples/x86itoa_src.build.shard
EVAL=bin/shard_eval
...
echo "ARTIFACT OK: do-while-fragment binaries + manifest + certs, all five gates green"
