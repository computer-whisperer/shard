...
# end for examples/wasmgen_src.shard. Five gates:
...
set -euo pipefail
SRC=examples/wasmgen_src.shard
OUT=examples/wasmgen_out.shard
EVAL=bin/shard_eval
TMP=$(mktemp -d); trap 'rm -rf "$TMP"' EXIT

echo "== gate 1: regen (producer determinism)"
"$EVAL" run tools/wasmgen/wasmgen.shard "$SRC" "$TMP/certs.raw" >/dev/null
...
"$EVAL" run tools/lowbuild/lowbuild.shard examples/wasmgen_src.build.shard > "$TMP/plan.txt"
