...
"${EMIT[@]}" run examples/riscv_diff_run.shard > "$TMP/plan.txt"

CFLAGS="-nostdlib -static -ffreestanding -fno-builtin -fno-stack-protector -Os -fuse-ld=$LD_LLD"
clang --target=riscv64-unknown-linux-gnu -march=rv64im $CFLAGS examples/riscv_diff.c -o "$TMP/rv64"
clang --target=riscv32-unknown-linux-gnu -march=rv32im $CFLAGS examples/riscv_diff.c -o "$TMP/rv32"
