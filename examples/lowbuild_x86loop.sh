#!/usr/bin/env bash
# lowbuild_x86loop.sh — the x86_64 LOOP-fragment artifact build (docs/X86.md
# §14): the mod.build convention end to end for examples/x86loop_src.shard,
...
set -euo pipefail
SRC=examples/x86loop_src.shard
OUT=examples/x86loop_out.shard
BUILD=examples/x86loop_src.build.shard
EVAL=bin/shard_eval
