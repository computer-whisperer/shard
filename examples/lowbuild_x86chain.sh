#!/usr/bin/env bash
# lowbuild_x86chain.sh — the x86_64 COMPOSITION-CHAIN artifact build (docs/X86.md
# §17): the mod.build convention end to end for examples/x86chain_src.shard,
...
#   3. KERNEL    — the cert proofs actually check (incl. the GENERATED
#                  clobber-set callee certs the callers cite)
#   4. BYTETIE   — tools/bytetie re-encodes each cert's module image at
#                  the claim's entry index (the x86 reflector + the
#                  flattening encoder, call rel32 included) and diffs
#                  against the plan's XMOD bytes, and the manifest's
#                  ARTIFACT lines bind name -> cert -> pinned export
#                  index (tools/lowcheck/manifest.shard)
...
set -euo pipefail
SRC=examples/x86chain_src.shard
OUT=examples/x86chain_out.shard
BUILD=examples/x86chain_src.build.shard
EVAL=bin/shard_eval
...
echo "== gate 4: byte tie (cert module images re-encode to the shipped bytes)"
...
echo "ARTIFACT OK: chain binaries + manifest + certs, all five gates green"
