...
#               at restfs := Nil, re-encoded by tools/bytetie, and the
#               manifest's ARTIFACT lines bind name -> cert -> pinned
#               export index (tools/lowcheck/manifest.shard)
...
"$EVAL" run tools/lowcheck/manifest.shard "$TMP/plan.txt" models/wasm/wasm.shard "$CERTS"
